...
- **v0.2.0-alpha.0** (2026-06-11) — Initial alpha release. `ActionProvider` + `Action`, four built-in plugins, plugin runtime, TypeScript registry augmentation, ESM build. `@drwyn/memory` (IndexedDB store + `memory`/`surface` plugins) and `@drwyn/client` (cloud SDK) ship alongside. Published under the `alpha` npm tag.
